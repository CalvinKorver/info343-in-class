...
describe("redux store", function() {
    ...
    /* Declares one particular step */
    it("should intialize with a default state", function() {
    ...
    it("should ignore an unkonwn action", function() {
        var state1 = store.getState();
        store.dispatch({type: "invalid action type"});
        var state2 = store.getState();
        expect(state1).to.equals(state2);
    });
});
